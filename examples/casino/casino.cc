...
const string _CasinostateId[] = {"start","dishonest","honest","end"};
const string _CasinoemissionId[] = {"empty","emitDishonest","emitHonest"};
const string _CasinotransitionId[] = {"id$12","id$13","id$14","id$15","id$16","id$17","id$18"};
    ...
    for (int iPos0=0; iPos0<iLen+1; ++iPos0) {
        ...
        if ((iPos0+0>=1)) {
            ...
            if ((iPos0+-1<=0)) {
                ...
                CurStateMemoryblock2To[1] = ((iTransition[0])*(iEmission[0]))*CurStateMemoryblock1From[0];
            }
            if ((iPos0+-1>=1)) {
                CurStateMemoryblock2From = dp.StateMemoryblock2.read((iPos0-(1))-(1));
                CurStateMemoryblock2To[1] += ((iTransition[1])*(iEmission[0]))*CurStateMemoryblock2From[1];
                CurStateMemoryblock2To[1] += ((iTransition[4])*(iEmission[0]))*CurStateMemoryblock2From[0];
                CurStateMemoryblock2To[0] = ((iTransition[2])*(iEmission[1]))*CurStateMemoryblock2From[1];
                CurStateMemoryblock2To[0] += ((iTransition[3])*(iEmission[1]))*CurStateMemoryblock2From[0];
            }
...
bfloat Backward(CasinoBaumWelch& bw,CasinoDPTable* pInTable,CasinoDPTable** ppOutTable,Params iPar,char *aSeq,int iLen) {
    ...
    for (int iPos0=(iLen+1)-1; iPos0>=0; --iPos0) {
        ...
        if ((iPos0+0>=1)) {
            ...
            if ((iPos0+1<=iLen+0)) {
                CurStateMemoryblock2From = dp.StateMemoryblock2.read((iPos0-(-1))-(1));
                CurStateMemoryblock2To[0] = iTempProb[1] = ((iTransition[3])*(iEmission[1]))*CurStateMemoryblock2From[0];
                ...
                CurStateMemoryblock2To[0] += iTempProb[1] = ((iTransition[4])*(iEmission[0]))*CurStateMemoryblock2From[1];
                iTempProb[1] *= CurStateMemoryblock2Secondary[0];
                bw.transitionBaumWelchCount0[4] += iTempProb[1];
                bw.emissionBaumWelchCount1[alphaIndexdice[iSymbol[0]]][1] += iTempProb[1];
                CurStateMemoryblock2To[1] = iTempProb[1] = ((iTransition[2])*(iEmission[1]))*CurStateMemoryblock2From[0];
                iTempProb[1] *= CurStateMemoryblock2Secondary[1];
                bw.transitionBaumWelchCount0[2] += iTempProb[1];
                bw.emissionBaumWelchCount1[alphaIndexdice[iSymbol[0]]][0] += iTempProb[1];
                CurStateMemoryblock2To[1] += iTempProb[1] = ((iTransition[1])*(iEmission[0]))*CurStateMemoryblock2From[1];
                iTempProb[1] *= CurStateMemoryblock2Secondary[1];
    ...
    for (int iPos0=(iLen+1)-1; iPos0>=0; --iPos0) {
        ...
        if ((iPos0+0>=1)) {
            ...
            if ((iPos0+1<=iLen+0)) {
                ...
                hmmocMaxInPlace( CurStateMemoryblock2To[0], ((iTransition[4])*(iEmission[1]))*CurStateMemoryblock2From[1] );
                CurStateMemoryblock2To[1] = ((iTransition[2])*(iEmission[0]))*CurStateMemoryblock2From[0];
                hmmocMaxInPlace( CurStateMemoryblock2To[1], ((iTransition[1])*(iEmission[1]))*CurStateMemoryblock2From[1] );
            }
            iEmission[0] = 1.0;
            if ((iPos0+0>=iLen+0)) {
                CurStateMemoryblock3From = dp.StateMemoryblock3.read();
                hmmocMaxInPlace( CurStateMemoryblock2To[0], ((iTransition[6])*(iEmission[0]))*CurStateMemoryblock3From[0] );
                hmmocMaxInPlace( CurStateMemoryblock2To[1], ((iTransition[5])*(iEmission[0]))*CurStateMemoryblock3From[0] );
            }
            dp.StateMemoryblock2.written();
        }
        if ((iPos0+0<=0)) {
...
const string _NoEmissionCasinotransitionId[] = {"id$44","id$45","id$46","id$47","id$48","id$49","id$50"};
